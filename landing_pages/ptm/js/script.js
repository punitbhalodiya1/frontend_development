...
function validateForm() {
    ...
    if (WhatsApp == "") {
        $('#WhatsApp_err_msg').text("Please enter WhatsApp");
        flag = 1;
    }else{
        $('#WhatsApp_err_msg').text("");
        //flag = 0;
    }

    /*var PersonName = $("#PersonName").val();
    if (PersonName == "") {
        $('#PersonName_err_msg').text("Please enter Person name who invite you");
        flag = 1;
    }else{
        $('#PersonName_err_msg').text("");
        //flag = 0;
    }*/
...
$(document).ready(function(){
    ...
    $('#google-sheet').on("submit",function(){
        ...
        if(validateForm()){
            $.ajax({
                 type : "POST",
                 url : "https://script.google.com/macros/s/AKfycbx1ZMeyK75mp3CdEfN7OzMo2Ft1D2IPA9x9ilHZH-x06OmdAyWIwnIKoVViVX5boeTCGg/exec",
                 data : $this.serialize(),
                 success: function(response) {
                        ...
                        // setTimeout(function() {
                        //     window.location.href = "https://gupshupcommunica8.com/ptm/thankyou.html";    
                        // }, 2000);
                    },
