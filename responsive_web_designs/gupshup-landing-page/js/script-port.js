...
function validateForm() {
    ...
    var name = $("#Name").val();
    if (name == "") {
        $('#name_err_msg').text("Por favor, insira seu nome");
        flag = 1;
    }else{
        $('#name_err_msg').text("");
        //flag = 0;
    ...
    var email = $("#Email").val();
    if (email == "") {
        $('#email_err_msg').text("Por favor endereço de e-mail");
        flag = 1;
    }else{
        if(IsEmail(email)==false){
            $('#email_err_msg').text("Por favor insira o endereço de e-mail válido");
            flag = 1;  
        }else{
            $('#email_err_msg').text("");
            //flag = 0;    
    ...
    var phone = $("#Phone").val();
    if (phone == "") {
        $('#phone_err_msg').text("Por favor insira o número do telefone");
        flag = 1;
    }else{
        if(validatePhone(phone) ==false){
            $('#phone_err_msg').text("Por favor insira um número de telefone válido");
            flag = 1;
        }else{
            $('#phone_err_msg').text("");
            //flag = 0;    
    ...
    if (country == "") {
        $('#con_err_msg').text("Por favor, selecione o país");
        flag = 1;
    }else{
        $('#con_err_msg').text("");
        //flag = 0;
    ...
    if (industry == "") {
        $('#ind_err_msg').text("Por favor, entre no setor");
        flag = 1;
    }else{
        $('#ind_err_msg').text("");
        //flag = 0;
        ...
        if(validateForm()){
            $.ajax({
                 ...
                 success: function(response) {
                        $('#final_msg').html("Obrigado por nos contatar ..! Entraremos em contato em breve...").addClass('success').removeClass('error'); 
